...
describe('PhysicsRegistry', () => {
  ...
  it('discovers cloth and rigid nodes and emits add events', () => {
    ...
    const clothDesc = events.find((e) => e.current?.type === 'cloth')!.current!
    expect(clothDesc.attrs.mass).toBe(0.5)
    const rigidDesc = events.find((e) => e.current?.type === 'rigid-static')!.current!
    expect(rigidDesc.tag).toBe('hero')
  ...
  it('emits update when layout or attrs change, and remove on detach', () => {
    ...
    registry.discover(document)
    const upd = events.find((e) => e.type === 'registry:update')
