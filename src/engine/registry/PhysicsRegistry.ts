...
export class PhysicsRegistry {
  ...
  private emit(e: RegistryEvent) {
    for (const fn of this.listeners) { try { fn(e) } catch (err) { console.error('PhysicsRegistry listener error:', err) } }
  }
  ...
  private describe(el: HTMLElement): PhysicsDescriptor | null {
    ...
    if (!type && dataType && (dataType === 'cloth' || dataType === 'rigid-dynamic' || dataType === 'rigid-static')) type = dataType as PhysicsType
    if (!type) return null
...
function parseNum(v?: string): number | undefined { if (!v) return undefined; const n = Number(v); return Number.isFinite(n) ? n : undefined }
